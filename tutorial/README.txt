...
for dir in /mount/britton/Erika/2024-07-22-Funannotate_life-gift/workflow-output/BIN_REASSEMBLY/*; do
  ...
  for file in "$dir"/reassembled_bins/*; do
    ...
    cp "$file" "${PWD}/${dirname}_${new_filename}"
  done
done
```

for dir in /mount/britton/Erika/2024-07-29-Funannotate_life-gift_batch_2/workflow-output/BIN_REASSEMBLY/*; do
  dirname=$(basename "$dir")
  for file in "$dir"/reassembled_bins/*; do
    filename=$(basename "$file")
    new_filename=$(echo "$filename" | awk -F. '{print $1"."$2".fa"}')
...
#notes for myself of order of operations
grab a node
update path
module load singularity
module load anaconda3
export EGGNOG_DATA_DIR=/mount/britton/Erika/databases/eggnog
source activate /mount/britton/kyle/conda_envs/eggnog_env
